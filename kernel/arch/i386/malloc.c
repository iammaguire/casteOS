...
void* liballoc_alloc(int count)
{
	...
	for(int i = 0; i < count; i++)
	{
		if(first_entry == 0)
		{
			first_entry = vmm_alloc_page(first_entry);
		}
		else
		{
			pt_entry* entry;
			vmm_alloc_page(entry);
		}
	}
...
int liballoc_free(void* addr, int count)
{
	for(int i = 0; i < count; i++)
		vmm_free_page((pt_entry*) addr + (i * 4096)); 
}
...
static inline int getexp( unsigned int size )
{
	...
	{
		#ifdef DEBUG
		printf("getexp returns -1 for %s less than MINEXP\n", itoa_nbuf(size, 10));
		#endif
...
static void dump_array()
{
	...
	printf("System memory allocated: %s\n", itoa_nbuf(l_allocated, 10));
	printf("Memory in used (malloc'ed): %s\n", itoa_nbuf(l_inuse, 10));

		for ( i = 0; i < MAXEXP; i++ )
		{
			printf("%s", itoa_nbuf(i, 10));
			printf("(%s): ", itoa_nbuf(l_completePages[i], 10));
